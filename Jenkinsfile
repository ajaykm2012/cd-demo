  ...
  node("docker-test") {
    ...
    stage("Integration Test") {
      try {
        ...
        sh "docker stop --filter label=com.docker.swarm.service.name=cd-demo --format "{{.ID}}""
        sh "docker rm -f --filter label=com.docker.swarm.service.name=cd-demo --format "{{.ID}}" || true"
